# online/ranker.py - полная версия с исправлениями
...
class Ranker:
    ...
    def __init__(self, models_provider, data_provider, config: Dict = None):
        self.models = models_provider  # ModelsProvider
        self.data = data_provider  # DataProvider
        ...
        self.personalization_weight = self.config.get('personalization_weight', 0.80)
    ...
    async def _compute_personalized_utility(self, candidate: Dict, context: Dict,
        ...
        """Вычисление персонализированной полезности"""
        ...
        if movie_genres and user_genres:
            ...
            if total_weight > 0:
                utility = utility * 0.5 + (genre_match_score / total_weight) * 0.5

        # Предсказание модели (исправлено)
        try:
    ...
    def _compute_recency(self, candidate: Dict) -> float:
        ...
        try:
            ...
            if not year or year == 'nan' or year == '':
                return 0.5
    ...
    def _aggregate_scores(self, scores: Dict, candidate: Dict, context: Dict) -> float:
        ...
        return final_score / total_weight if total_weight > 0 else final_score
    ...
    def _apply_diversity(self, candidates: List[Dict], context: Dict) -> List[Dict]:
        ...
        for candidate in candidates:
            ...
            if len(diversified) >= self.final_top_n * 2:
                break
    ...
    def _get_movie_similarity(self, movie_id1: str, movie_id2: str) -> float:
        ...
        try:
            ...
            logger.debug(f"Ошибка получения схожести: {e}")
            return 0